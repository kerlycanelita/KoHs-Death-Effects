...
public final class KidsEffectManager {
	...
	private void addDoll(Minecraft client, UUID carrierUuid, UUID killUuid, UUID victimUuid, String victimName, LocalSettings settings) {
		...
		if (state.dolls.size() >= limit) {
			if (settings.mode() == KidsMode.CUMULATIVE && client.player != null && carrierUuid.equals(client.player.getUUID())) {
				client.gui.setOverlayMessage(Component.literal("Kids limit reached: 20 players"), false);
			}
			return;
		}
	...
	private void render(LevelRenderContext context) {
		...
		if (client.level == null || matrices == null || consumers == null || this.carriers.isEmpty()) {
			return;
		}
	...
	private void renderDoll(
		...
		int alphaInt = Mth.clamp((int)(alpha * 255.0F), 0, 255);
		VertexConsumer vertices = context.bufferSource().getBuffer(RenderTypes.entityTranslucent(doll.skin.body().texturePath(), false));
		PoseStack matrices = context.poseStack();
		matrices.pushPose();
