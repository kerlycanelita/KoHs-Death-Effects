...
public final class KohsDeathEffectsConfigScreen extends Screen {
	...
	private void initSoundTab() {
		...
		this.addStepButtons(controlX, y, () -> {
			this.config.customDeathSoundVolume = Math.max(0.0F, this.config.customDeathSoundVolume - 0.1F);
			this.config.save();
		}, () -> {
			this.config.customDeathSoundVolume = Math.min(3.0F, this.config.customDeathSoundVolume + 0.1F);
			this.config.save();
		});
	...
	public void removed() {
		if (this.previewPlayer != null) {
			KohsDeathEffectsClient.getKidsEffectManager().clearPreviewShoulders(this.previewPlayer.getId());
		}
	...
	private void initRagdollOptions() {
		...
		if (this.config.faintAnimationType == KohsDeathEffectsConfig.FaintAnimationType.CRAWL) {
			y += ROW_STEP;
			this.addStepButtons(controlX, y, () -> {
				this.config.faintCrawlSpeed = Math.max(100, this.config.faintCrawlSpeed - 10);
				this.config.save();
			}, () -> {
				this.config.faintCrawlSpeed = Math.min(300, this.config.faintCrawlSpeed + 10);
				this.config.save();
			});
		}
	}
	...
	private void initKidsOptions() {
		...
		this.addStepButtons(controlX, y, () -> {
			if (this.config.kidsMode == KohsDeathEffectsConfig.KidsMode.CUMULATIVE) {
				this.config.kidsCumulativeTimerSeconds = Math.max(minimumTimer, currentTimer - 1);
			} else {
				this.config.kidsTimerSeconds = Math.max(minimumTimer, currentTimer - 1);
			}
			this.config.save();
		}, () -> {
			if (this.config.kidsMode == KohsDeathEffectsConfig.KidsMode.CUMULATIVE) {
				this.config.kidsCumulativeTimerSeconds = Math.min(301, currentTimer + 1);
			} else {
				this.config.kidsTimerSeconds = Math.min(301, currentTimer + 1);
			}
			this.config.save();
		});
		...
		if (this.config.kidsMode == KohsDeathEffectsConfig.KidsMode.CUMULATIVE) {
			y += ROW_STEP;
			this.addStepButtons(controlX, y, () -> {
				this.config.kidsRopeSizePercent = Math.max(100, this.config.kidsRopeSizePercent - 10);
				this.config.save();
			}, () -> {
				this.config.kidsRopeSizePercent = Math.min(300, this.config.kidsRopeSizePercent + 10);
				this.config.save();
			});
			y += ROW_STEP * 2 + 20;
	...
	private void drawRagdollOptions(DrawContext context) {
		...
		if (this.config.faintAnimationType == KohsDeathEffectsConfig.FaintAnimationType.CRAWL) {
			...
			this.drawValue(context, this.config.faintCrawlSpeed + "%", y);
		}
	}
	...
	private void drawKidsOptions(DrawContext context) {
		...
		int timer = this.config.kidsMode == KohsDeathEffectsConfig.KidsMode.CUMULATIVE
			? this.config.kidsCumulativeTimerSeconds
			: this.config.kidsTimerSeconds;
		this.drawValue(context, kidsTimerText(timer), y);
		if (this.config.kidsMode == KohsDeathEffectsConfig.KidsMode.CUMULATIVE) {
			y += ROW_STEP;
			this.drawChildLabel(context, "Rope Size", y);
			this.drawValue(context, this.config.kidsRopeSizePercent + "%", y);
			y += ROW_STEP;
	...
	private void drawSoundOptions(DrawContext context) {
		...
		y += ROW_STEP;
		this.drawChildLabel(context, "Volume", y);
		this.drawValue(context, Math.round(this.config.customDeathSoundVolume * 100.0F) + "%", y);
	...
	private void drawPlayerPreview(DrawContext context, int left, int top, int right, int bottom, float progress) {
		...
		context.fill(left, previewTop, right, previewBottom, withAlpha(coverAlpha, 0x0B1014));
	}
	...
	private void drawRagdollPreview(DrawContext context, int left, int top, int right, int bottom, float progress) {
		...
		float crawlCycle = crawlAmount > 0.0F && progress < 0.82F
			...
			: 0.0F;
		model.animateModel(entity, 0.0F, 0.0F, 1.0F);
		model.setAngles(entity, 0.0F, 0.0F, entity.age, 0.0F, 0.0F);
		...
		if (coverAlpha > 0) {
			context.fill(left, top, right, bottom, withAlpha(coverAlpha, 0x0B1014));
		}
	}
	...
	private void drawKidsPreviewImage(DrawContext context, int left, int top, int right, int bottom) {
		...
		context.drawGuiTexture(KIDS_PREVIEW_TEXTURE_ID, imageX, imageY, imageWidth, imageHeight);
	}
	...
	private void drawKidsPreviewLegacy(DrawContext context, int left, int top, int right, int bottom, float progress) {
		...
		KohsDeathEffectsClient.getKidsEffectManager().setPreviewShoulders(carrier.getId(), this.currentPreviewSkin(), wave);
		int size = MathHelper.clamp((int)(Math.min(right - left, bottom - top) * 0.48F * this.morphPreviewZoom), 18, 86);
		InventoryScreen.drawEntity(
			context,
			left,
			top,
	...
	private void drawSilhouetteModel(DrawContext context, int left, int top, int right, int bottom, float scale, int alpha) {
		...
		this.drawPlayerSkin(
			...
			right,
			bottom,
			withAlpha(alpha, this.config.silhouetteColor)
		);
	}
	...
	private enum EffectTab {
		SILHOUETTE("Silhouette", KohsDeathEffectsConfig.DeathEffectMode.SILHOUETTE),
		PLAYER("Player", KohsDeathEffectsConfig.DeathEffectMode.PLAYER_GHOST),
		RAGDOLL("Faint", KohsDeathEffectsConfig.DeathEffectMode.RAGDOLL),
		KIDS("Kids", KohsDeathEffectsConfig.DeathEffectMode.KIDS),
	...
	private static final class MorphToScreen extends Screen {
		...
		@Override
		public boolean mouseClicked(double mouseX, double mouseY, int button) {
			if (this.handleMobRowClick(mouseX, mouseY)) {
				return true;
			}
