...
public final class KidsEffectManager {
	...
	private PlayerEntityModel<AbstractClientPlayerEntity> regularModel;
	...
	private void addDoll(MinecraftClient client, UUID carrierUuid, UUID killUuid, UUID victimUuid, String victimName, LocalSettings settings) {
		...
		DollState doll = new DollState(killUuid, victimUuid, profile.getName(), fallback, settings.lifetimeTicks());
		AbstractClientPlayerEntity onlineVictim = findPlayer(client, victimUuid);
		if (onlineVictim != null) {
			doll.skin = onlineVictim.getSkinTextures();
		} else {
			client.getSkinProvider().fetchSkinTextures(profile).thenAccept(result -> client.execute(() -> doll.skin = result));
		}
	...
	void renderShoulderDolls(
		...
		int light,
		AbstractClientPlayerEntity carrier,
		PlayerEntityModel<AbstractClientPlayerEntity> carrierModel,
		float tickDelta
	) {
		PreviewShoulderState preview = this.previewShoulderState;
		if (preview != null && carrier.getId() == preview.entityId()) {
	...
	private void submitShoulderDoll(
		...
		PlayerEntityModel<AbstractClientPlayerEntity> dollModel = this.getModel(skin.model() == SkinTextures.Model.SLIM);
		this.prepareModel(dollModel, carrier, carrier.age, true, wave);
		VertexConsumer vertices = consumers.getBuffer(RenderLayer.getEntityTranslucent(skin.texture(), false));
		matrices.push();
		carrierModel.body.rotate(matrices);
		...
		dollModel.render(matrices, vertices, light, OverlayTexture.DEFAULT_UV, argb(alpha, 255, 255, 255));
		matrices.pop();
	}
	...
	private void render(WorldRenderContext context) {
		...
		float tickProgress = context.tickCounter().getTickDelta(false);
	...
	private void renderCarrierKids(WorldRenderContext context, AbstractClientPlayerEntity carrier, CarrierState state, Vec3d cameraPos, float tickProgress) {
		...
		float bodyYaw = MathHelper.lerpAngleDegrees(tickProgress, carrier.prevBodyYaw, carrier.bodyYaw);
	...
	private void renderDoll(
		...
		AbstractClientPlayerEntity poseEntity = findPlayer(MinecraftClient.getInstance(), doll.victimUuid);
		if (poseEntity == null) {
			poseEntity = MinecraftClient.getInstance().player;
		}
		if (poseEntity == null) {
			return;
		}
		PlayerEntityModel<AbstractClientPlayerEntity> model = this.getModel(doll.skin.model() == SkinTextures.Model.SLIM);
		this.prepareModel(model, poseEntity, doll.ageTicks, shoulder, animation);

		float spawnScale = 0.65F + smoothStep(MathHelper.clamp(doll.ageTicks / 10.0F, 0.0F, 1.0F)) * 0.35F;
		float scale = baseScale * spawnScale;
		int alphaInt = MathHelper.clamp((int)(alpha * 255.0F), 0, 255);
		VertexConsumer vertices = context.consumers().getBuffer(RenderLayer.getEntityTranslucent(doll.skin.texture(), false));
		MatrixStack matrices = context.matrixStack();
		matrices.push();
	...
	private PlayerEntityModel<AbstractClientPlayerEntity> getModel(boolean slim) {
		if (slim) {
			if (this.slimModel == null) {
				this.slimModel = new PlayerEntityModel<>(MinecraftClient.getInstance().getEntityModelLoader().getModelPart(EntityModelLayers.PLAYER), true);
			}
			return this.slimModel;
		}

		if (this.regularModel == null) {
			this.regularModel = new PlayerEntityModel<>(MinecraftClient.getInstance().getEntityModelLoader().getModelPart(EntityModelLayers.PLAYER), false);
		}
		return this.regularModel;
	}

	private void prepareModel(PlayerEntityModel<AbstractClientPlayerEntity> model, AbstractClientPlayerEntity entity, float age, boolean shoulder, float animation) {
		model.handSwingProgress = 0.0F;
		model.riding = false;
		model.child = false;
		model.setAngles(entity, 0.0F, 0.0F, age, 0.0F, 0.0F);
		if (shoulder) {
			applyShoulderPose(model, animation);
		} else {
			applyDraggedPose(model, animation);
		}
		syncOverlayTransforms(model);
		showAllParts(model);
	}
	...
	private static void syncOverlayTransforms(PlayerEntityModel model) {
	...
	private static net.minecraft.client.world.ClientWorld clientWorld(AbstractClientPlayerEntity player) {
		return (net.minecraft.client.world.ClientWorld)player.getWorld();
	}
