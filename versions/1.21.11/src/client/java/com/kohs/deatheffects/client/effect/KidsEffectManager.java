...
public final class KidsEffectManager {
	...
	private void submitShoulderDoll(
		...
		commandQueue.submitModel(
			...
			dollRenderState,
			matrices,
			RenderLayers.entityTranslucent(skin.body().texturePath(), false),
			light,
	...
	private void renderDoll(
		...
		int alphaInt = MathHelper.clamp((int)(alpha * 255.0F), 0, 255);
		VertexConsumer vertices = context.consumers().getBuffer(RenderLayers.entityTranslucent(doll.skin.body().texturePath(), false));
		MatrixStack matrices = context.matrices();
		matrices.push();
	...
	private void renderRope(WorldRenderContext context, List<RopeSegment> segments, Vec3d cameraPos, float sizeMultiplier) {
		...
		VertexConsumer vertices = context.consumers().getBuffer(RenderLayers.linesTranslucent());
