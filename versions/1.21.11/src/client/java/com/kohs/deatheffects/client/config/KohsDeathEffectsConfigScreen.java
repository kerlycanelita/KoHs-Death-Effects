...
public final class KohsDeathEffectsConfigScreen extends Screen {
	...
	private static final Identifier KIDS_PREVIEW_TEXTURE_ID = Identifier.of(KohsDeathEffects.MOD_ID, "kids_preview");
	private static final Identifier PREVIEW_WORLD_TEXTURE_ID = Identifier.of(KohsDeathEffects.MOD_ID, "textures/gui/preview_world.png");
	...
	private PlayerEntityModel kidsDraggedPreviewModel;
	...
	protected void init() {
		...
		int doneWidth = this.panelWidth < 360 ? 64 : 82;
		this.addDrawableChild(this.purpleButton(Text.literal("Done"), button -> this.close(),
			this.panelX + this.panelWidth - doneWidth - 14,
			this.panelY + this.panelHeight - this.footerHeight + 7,
			doneWidth,
			20,
			ButtonTone.PRIMARY));
	}
	...
	private void initSoundTab() {
		...
		y += ROW_STEP;
		this.addScrolledDrawableChild(this.purpleButton(Text.literal("Folder"), button -> DeathSoundManager.openCustomSoundDirectory(),
			controlX - 112, y, 104, 20, ButtonTone.NORMAL));
		this.addScrolledDrawableChild(this.purpleButton(Text.literal("Reload"), button -> {
			DeathSoundManager.refresh();
			this.clearAndInit();
		...
		y += ROW_STEP;
		y += 24;
		this.addScrolledDrawableChild(this.purpleButton(Text.literal("None"), button -> {
	...
	private void applyCustomDeathSound(String soundId) {
		...
		if (soundId != null && this.client != null && this.client.player != null) {
			DeathSoundManager.playPreviewAt(entityPosition(this.client.player));
		}
	}
	...
	public boolean mouseScrolled(double mouseX, double mouseY, double horizontalAmount, double verticalAmount) {
		if (this.previewCanOrbit() && this.isMouseOverPreview(mouseX, mouseY)) {
			this.morphPreviewZoom = MathHelper.clamp(this.morphPreviewZoom + (float)verticalAmount * 0.10F, PREVIEW_MIN_ZOOM, PREVIEW_MAX_ZOOM);
			return true;
		}
	...
	private void addPreviewControls() {
		...
		if (this.currentEffectTab == EffectTab.KIDS) {
			...
			this.addDrawableChild(this.purpleButton(Text.literal(this.previewPaused ? "Play" : "Pause"), button -> {
				this.togglePreviewPaused();
				button.setMessage(Text.literal(this.previewPaused ? "Play" : "Pause"));
			}, firstX, buttonY, kidsButtonWidth, 20, ButtonTone.SMALL));
			this.addDrawableChild(this.purpleButton(Text.literal("Center"), button -> this.resetPreviewView(),
				firstX + kidsButtonWidth + gap, buttonY, kidsButtonWidth, 20, ButtonTone.SMALL));
			return;
		}

		int buttonWidth = Math.max(30, (availableWidth - gap * 2) / 3);
		this.addDrawableChild(this.purpleButton(Text.literal("Replay"), button -> {
			this.previewPaused = false;
			this.restartPreview();
			this.clearAndInit();
		...
		if (this.currentEffectTab == EffectTab.MORPH) {
			this.addDrawableChild(this.purpleButton(Text.literal("Sound"), button -> this.playPreviewMorphSound(),
	...
	private void playPreviewMorphSound() {
		...
		MorphMobSoundPlayer.playConfigured(entityPosition(this.client.player), this.config);
	}
	...
	private void initSilhouetteOptions() {
		...
		int fieldWidth = MathHelper.clamp(this.optionsWidth - CHILD_INDENT, 104, 160);
		int fieldX = this.optionsX + CHILD_INDENT;
		int fieldY = y + 18;
		this.colorField = new TextFieldWidget(this.textRenderer, fieldX, fieldY, fieldWidth, 20, Text.literal("Color"));
	...
	private void initRagdollOptions() {
		...
		int halfWidth = MathHelper.clamp((this.optionsWidth - CHILD_INDENT - 8) / 2, 70, 112);
		int firstX = this.optionsX + CHILD_INDENT;
		int secondX = firstX + halfWidth + 8;
		this.addScrolledDrawableChild(this.purpleButton(Text.literal("Fall"), button -> {
		...
		this.addScrolledDrawableChild(this.purpleButton(Text.literal("Crawl"), button -> {
	...
	private void initKidsOptions() {
		...
		int firstX = this.optionsX + CHILD_INDENT;
		int secondX = firstX + halfWidth + 8;
		this.addScrolledDrawableChild(this.purpleButton(Text.literal("Cumulative"), button -> {
		...
		this.addScrolledDrawableChild(this.purpleButton(Text.literal("Only Shoulders"), button -> {
		...
		if (this.config.kidsMode == KohsDeathEffectsConfig.KidsMode.CUMULATIVE) {
			...
			y += ROW_STEP + 20;
			this.addScrolledDrawableChild(this.purpleButton(Text.literal("Look Down"), button -> {
			...
			this.addScrolledDrawableChild(this.purpleButton(Text.literal("Look Up"), button -> {
	...
	private void initAdvancedTab() {
		int controlX = this.optionsX + this.optionsWidth - 114;
		int y = this.logicalOptionsTop() + ROW_STEP;
		this.addScrolledDrawableChild(this.purpleButton(Text.literal("Reset"), button -> {
	...
	private void addIntSlider(int y, int initialValue, int minimum, int maximum, int step, IntConsumer onValueChanged, IntFunction<String> formatter) {
		int width = MathHelper.clamp(this.optionsWidth * 55 / 100, 104, 260);
	...
	private PurpleButtonWidget purpleButton(Text message, PurplePressAction onPress, int x, int y, int width, int height, ButtonTone tone) {
	...
	private void drawFrame(DrawContext context) {
		...
		context.drawStrokedRectangle(this.panelX + 3, this.panelY + 3, this.panelWidth - 6, this.panelHeight - 6, 0x663D2367);
		...
		int selectedX = this.panelX + horizontalPadding + this.currentTab.ordinal() * (tabWidth + tabGap);
		context.fill(selectedX - 2, this.panelY + 26, selectedX + tabWidth + 2, this.panelY + 50, COLOR_PURPLE_SOFT);
	...
	private void drawAnimatedParticleBackground(DrawContext context) {
		context.fill(0, 0, this.width, this.height, COLOR_SCREEN_SHADE);
		...
		for (int index = 0; index < BACKGROUND_PARTICLE_COUNT; index++) {
			...
			int x = Math.floorMod(seed, Math.max(1, this.width + 32)) - 16;
			x += Math.round(MathHelper.sin(progress * (float)Math.PI * 2.0F + (seed & 31)) * (3.0F + Math.floorMod(seed >>> 13, 9)));
			int y = this.height + 12 - Math.round(progress * (this.height + 36));
			int size = 1 + Math.floorMod(seed >>> 18, 3);
			float pulse = 0.45F + 0.55F * MathHelper.sin(progress * (float)Math.PI);
			int alpha = MathHelper.clamp(Math.round((38 + Math.floorMod(seed >>> 23, 86)) * pulse), 18, 128);
			int rgb = (index & 3) == 0 ? 0xD8A7FF : (index & 3) == 1 ? 0xB96BFF : 0x7C3CFF;
			if (size >= 2) {
				context.fill(x - 1, y - 1, x + size + 1, y + size + 1, ColorHelper.withAlpha(Math.max(8, alpha / 4), rgb));
			}
			context.fill(x, y, x + size, y + size, ColorHelper.withAlpha(alpha, rgb));
		}
	}
	...
	private void drawContent(DrawContext context, int mouseX, int mouseY) {
		if (this.currentTab == MainTab.EFFECTS) {
			...
			context.drawText(this.textRenderer, Text.literal("Advanced"), this.optionsX, this.contentY, COLOR_TEXT, false);
	...
	private void drawEffectOptions(DrawContext context) {
		if (this.sidebarWidth > 0) {
			context.drawText(this.textRenderer, Text.literal(this.currentEffectTab.label + " Settings"), this.optionsX, this.contentY, COLOR_TEXT, false);
		}
	...
	private void drawSilhouetteOptions(DrawContext context) {
		...
		context.drawStrokedRectangle(fieldX - 2, fieldY - 2, fieldWidth + 4, 24, COLOR_BORDER);
	...
	private void drawKidsOptions(DrawContext context) {
		int y = this.scrolledY(this.logicalOptionsTop() + 5);
	...
	private void drawSoundOptions(DrawContext context) {
		...
		if (DeathSoundManager.getSoundFiles().isEmpty()) {
			y += 24;
			context.drawText(this.textRenderer, Text.literal("No .mp3 files found"), this.optionsX + CHILD_INDENT, this.scrolledY(this.logicalOptionsTop() + 5 + ROW_STEP * 3 + 24), COLOR_TEXT_DIM, false);
		}

		String selectedName = selectedSoundName();
		if (!selectedName.isBlank()) {
			context.drawText(this.textRenderer, Text.literal("Selected: " + trimButtonLabel(selectedName, 28)), this.optionsX + CHILD_INDENT, this.optionsScrollBottom - 12, COLOR_TEXT_MUTED, false);
		}
		context.disableScissor();
	}
	...
	private void drawPreview(DrawContext context, int mouseX, int mouseY) {
		...
		context.drawStrokedRectangle(this.previewX, this.previewY, this.previewWidth, this.previewHeight, COLOR_BORDER);
	...
	private void drawPreviewWorldBackground(DrawContext context, int left, int top, int right, int bottom) {
		...
		int sourceX = 0;
		int sourceY = 0;
		int sourceWidth = PREVIEW_WORLD_IMAGE_WIDTH;
		int sourceHeight = PREVIEW_WORLD_IMAGE_HEIGHT;
		if (viewportAspect < imageAspect) {
			sourceWidth = Math.max(1, Math.round(PREVIEW_WORLD_IMAGE_HEIGHT * viewportAspect));
			sourceX = (PREVIEW_WORLD_IMAGE_WIDTH - sourceWidth) / 2;
		} else {
			sourceHeight = Math.max(1, Math.round(PREVIEW_WORLD_IMAGE_WIDTH / viewportAspect));
			sourceY = (PREVIEW_WORLD_IMAGE_HEIGHT - sourceHeight) / 2;
		}
		context.drawTexture(
			RenderPipelines.GUI_TEXTURED,
			PREVIEW_WORLD_TEXTURE_ID,
			left,
			top,
			(float)sourceX,
			(float)sourceY,
			...
			sourceWidth,
			sourceHeight,
			PREVIEW_WORLD_IMAGE_WIDTH,
			PREVIEW_WORLD_IMAGE_HEIGHT
		);
		context.fill(left, top, right, bottom, 0x300A0711);
	}

	private int previewFooterSpace() {
		return this.previewHeight >= 148 ? 52 : this.previewHeight >= 74 ? 32 : 8;
	}
	...
	private void drawPreviewHelp(DrawContext context) {
		if (this.previewWidth <= 0 || this.previewHeight < 148) {
			return;
		}

		String text = this.currentEffectTab == EffectTab.KIDS
			? "Shoulders and train use the selected settings"
			: "Drag: rotate | Wheel: zoom";
		context.drawText(
			this.textRenderer,
			Text.literal(this.textRenderer.trimToWidth(text, Math.max(40, this.previewWidth - 16))),
			this.previewX + 8,
	...
	private void drawGifPlaceholder(DrawContext context, int left, int top, int right, int bottom, String label) {
		...
		context.drawStrokedRectangle(left + 8, top + 8, Math.max(20, width - 16), Math.max(20, height - 16), 0x889D63FF);
		...
		if (height >= 56) {
			context.drawCenteredTextWithShadow(this.textRenderer, Text.literal("here"), centerX, centerY + 8, COLOR_TEXT_DIM);
		}
	}
	...
	private void drawSilhouettePreview(DrawContext context, int left, int top, int right, int bottom, float progress) {
		...
		int alpha = MathHelper.clamp((int)(fade * 255.0F), 0, 255);
		int width = right - left;
		int height = bottom - top;
		float rise = this.config.risingSilhouetteEnabled
			? this.previewFrame == null ? easeOutCubic(progress) : this.previewFrame.riseProgress()
			: 0.0F;
	...
	private void drawPlayerPreview(DrawContext context, int left, int top, int right, int bottom, float progress) {
		...
		float rise = this.config.playerGhostEnabled && rising
			? this.previewFrame == null ? easeOutCubic(progress) : this.previewFrame.riseProgress()
			: 0.0F;
		int width = right - left;
		int height = bottom - top;
	...
	private void drawRagdollPreview(DrawContext context, int left, int top, int right, int bottom, float progress) {
		...
		SkinTextures skin = this.currentPreviewSkin();
		float elapsedTicks = this.previewFrame == null ? progress * 120.0F : this.previewFrame.elapsedTicks();
		float fall = this.previewFrame == null ? MathHelper.clamp(elapsedTicks / 26.0F, 0.0F, 1.0F) : this.previewFrame.faintFall();
		float settle = this.previewFrame == null ? MathHelper.clamp((elapsedTicks - 26.0F) / 16.0F, 0.0F, 1.0F) : this.previewFrame.faintSettle();
		...
		float crawlCycle = this.previewFrame == null
			? crawlAmount > 0.0F && progress < 0.82F
				? MathHelper.sin(elapsedTicks * (0.34F + this.config.faintCrawlSpeed / 260.0F))
				: 0.0F
		...
		float scale = MathHelper.clamp(Math.min(horizontalFit, verticalFit) * this.morphPreviewZoom, 22.0F, 124.0F);
		...
		context.addPlayerSkin(
			...
			scale,
			5.0F + MathHelper.clamp(this.morphPreviewPitchOffset, -16.0F, 16.0F),
			30.0F + MathHelper.clamp(this.morphPreviewYawOffset, -38.0F, 38.0F),
			0.92F,
			cameraLeft,
			cameraTop,
			cameraRight,
	...
	private void drawKidsPreview(DrawContext context, int left, int top, int right, int bottom, float progress) {
		this.drawKidsPreviewLegacy(context, left, top, right, bottom, progress);
	}

	private void drawKidsPreviewImage(DrawContext context, int left, int top, int right, int bottom) {
		...
		context.drawGuiTexture(RenderPipelines.GUI_TEXTURED, KIDS_PREVIEW_TEXTURE_ID, imageX, imageY, imageWidth, imageHeight);
	}

	private void drawKidsPreviewLegacy(DrawContext context, int left, int top, int right, int bottom, float progress) {
		...
		int carrierSize = MathHelper.clamp((int)(Math.min(width, height) * carrierFactor * this.morphPreviewZoom), 16, 70);
		float wave = this.config.kidsAnimationEnabled ? MathHelper.sin(progress * (float)Math.PI * 8.0F) : 0.0F;
		KohsDeathEffectsClient.getKidsEffectManager().setPreviewShoulders(KIDS_PREVIEW_ENTITY_ID, this.currentPreviewSkin(), wave);

		float ropeSizeMultiplier = MathHelper.clamp(this.config.kidsRopeSizePercent / 100.0F, 1.0F, 3.0F);
		...
		int draggedCenterY = bottom - MathHelper.clamp(Math.round(height * 0.18F), 6, 16);
		if (showTrain) {
			int ropeAnchorX = centerX + MathHelper.clamp(Math.round(carrierSize * 0.24F), 3, 12);
			int ropeAnchorY = bottom - MathHelper.clamp(Math.round(carrierSize * 0.38F), 6, 18);
			int ropeThickness = MathHelper.clamp(Math.round(ropeSizeMultiplier), 1, 3);
		...
		if (showTrain) {
			PlayerEntityModel draggedModel = this.getKidsDraggedPreviewModel();
			PlayerEntityRenderState draggedState = this.createRagdollPreviewState();
			draggedModel.setAngles(draggedState);
			float frantic = this.config.kidsAnimationEnabled && this.config.kidsDraggedHandMovementEnabled
				? MathHelper.sin(progress * (float)Math.PI * 12.0F)
				: 0.0F;
			...
			float draggedScale = MathHelper.clamp(
	...
	private static void drawPreviewRope(DrawContext context, int fromX, int fromY, int toX, int toY, int thickness) {
	...
	private void drawSilhouetteModel(DrawContext context, int left, int top, int right, int bottom, float scale, int alpha) {
		...
		int tintAlpha = MathHelper.clamp((int)(alpha * 0.68F), 0, 190);
		context.addPlayerSkin(
			model,
			getSilhouettePreviewTexture(ColorHelper.withAlpha(tintAlpha, this.config.silhouetteColor)),
			scale,
			pitch,
			yaw,
			1.6F,
	...
	private PlayerEntityRenderState createSilhouettePreviewState() {
		...
		state.playerName = Text.literal(this.currentPlayerName());
	...
	private PlayerEntityModel getSilhouettePreviewModel() {
		boolean slim = this.currentPreviewSkin().model() == PlayerSkinType.SLIM;
		if (this.silhouettePreviewModel == null || this.silhouettePreviewModelSlim != slim) {
			this.silhouettePreviewModel = new PlayerEntityModel(
				MinecraftClient.getInstance().getLoadedEntityModels().getModelPart(EntityModelLayers.PLAYER),
				slim
			);
	...
	private PlayerEntityModel getKidsDraggedPreviewModel() {
		boolean slim = this.currentPreviewSkin().model() == PlayerSkinType.SLIM;
		if (this.kidsDraggedPreviewModel == null || this.kidsDraggedPreviewModelSlim != slim) {
			this.kidsDraggedPreviewModel = new PlayerEntityModel(
				MinecraftClient.getInstance().getLoadedEntityModels().getModelPart(EntityModelLayers.PLAYER),
				slim
			);
	...
	private SkinTextures currentPreviewSkin() {
		if (this.client != null && this.client.player != null) {
			return this.client.player.getSkin();
		}
	...
	private static void applyFaintPreviewPose(PlayerEntityModel model, float fall, float settle, float crawlCycle, float crawlAmount) {
		...
		model.head.pitch = MathHelper.lerp(grounded, 0.24F * limp, MathHelper.lerp(crawlAmount, 0.34F, -0.18F));
		model.head.yaw = MathHelper.lerp(grounded, 0.0F, MathHelper.lerp(crawlAmount, -0.42F, bodySway * 0.10F));
		model.head.roll = MathHelper.lerp(grounded, 0.0F, MathHelper.lerp(crawlAmount, 0.10F, -bodySway * 0.05F));
		model.body.pitch = MathHelper.lerp(grounded, 0.10F * limp, MathHelper.lerp(crawlAmount, 0.03F, -0.08F + Math.abs(crawlCycle) * 0.04F));
		model.body.roll = MathHelper.lerp(grounded, 0.0F, MathHelper.lerp(crawlAmount, -0.04F, bodySway * 0.07F));
		model.rightArm.pitch = MathHelper.lerp(crawlAmount, MathHelper.lerp(grounded, 0.70F + limp * 0.25F, 1.20F), -1.20F + rightPull * 0.58F);
		model.rightArm.yaw = MathHelper.lerp(crawlAmount, MathHelper.lerp(grounded, -0.16F, -0.48F), -0.58F + rightPull * 0.28F);
		model.rightArm.roll = MathHelper.lerp(crawlAmount, MathHelper.lerp(grounded, 0.12F, 0.26F), 0.44F - rightPull * 0.20F);
		model.leftArm.pitch = MathHelper.lerp(crawlAmount, MathHelper.lerp(grounded, 0.68F + limp * 0.24F, 1.16F), -1.20F + leftPull * 0.58F);
		model.leftArm.yaw = MathHelper.lerp(crawlAmount, MathHelper.lerp(grounded, 0.16F, 0.46F), 0.58F - leftPull * 0.28F);
		model.leftArm.roll = MathHelper.lerp(crawlAmount, MathHelper.lerp(grounded, -0.12F, -0.25F), -0.44F + leftPull * 0.20F);
		model.rightLeg.pitch = MathHelper.lerp(crawlAmount, MathHelper.lerp(grounded, -0.08F, 0.10F), 0.32F - rightPull * 0.12F);
		model.rightLeg.yaw = MathHelper.lerp(crawlAmount, MathHelper.lerp(grounded, 0.06F, 0.18F), 0.16F + rightPull * 0.12F);
		model.rightLeg.roll = MathHelper.lerp(crawlAmount, MathHelper.lerp(grounded, 0.03F, 0.08F), 0.05F);
		model.leftLeg.pitch = MathHelper.lerp(crawlAmount, MathHelper.lerp(grounded, 0.08F, -0.08F), 0.32F - leftPull * 0.12F);
		model.leftLeg.yaw = MathHelper.lerp(crawlAmount, MathHelper.lerp(grounded, -0.06F, -0.16F), -0.16F - leftPull * 0.12F);
		model.leftLeg.roll = MathHelper.lerp(crawlAmount, MathHelper.lerp(grounded, -0.03F, -0.07F), -0.05F);
		resetPlayerOverlayTransforms(model);
	}

	private static void applyKidsDraggedPose(PlayerEntityModel model, float frantic) {
	...
	private static void applyKidsDraggedPreviewOrientation(PlayerEntityModel model) {
	...
	private static void orientFaintPreviewHorizontally(PlayerEntityModel model) {
	...
	private static void rotatePreviewPart(net.minecraft.client.model.ModelPart part, float angle) {
		float pivotY = 12.0F;
		float x = part.originX;
		float y = part.originY - pivotY;
		float cosine = MathHelper.cos(angle);
		float sine = MathHelper.sin(angle);
		part.originX = x * cosine - y * sine;
		part.originY = pivotY + x * sine + y * cosine;
		part.roll += angle;
	}
	...
	private PreviewPlayerEntity getPreviewPlayer() {
		...
		if (this.previewPlayer == null || this.previewPlayer.getEntityWorld() != this.client.world) {
	...
	private LivingEntity getPreviewMorphEntity() {
		...
		if (this.previewMorphEntity == null || !mobId.equals(this.previewMorphEntityId) || this.previewMorphEntity.getEntityWorld() != this.client.world) {
	...
	private void preparePreviewPlayer(PreviewPlayerEntity entity) {
		...
		LivingEntity source = this.client == null ? null : this.client.player;
		entity.equipStack(EquipmentSlot.HEAD, this.copyEquipment(source, EquipmentSlot.HEAD, this.config.playerGhostArmorEnabled));
		entity.equipStack(EquipmentSlot.CHEST, this.copyEquipment(source, EquipmentSlot.CHEST, this.config.playerGhostArmorEnabled));
		entity.equipStack(EquipmentSlot.LEGS, this.copyEquipment(source, EquipmentSlot.LEGS, this.config.playerGhostArmorEnabled));
		entity.equipStack(EquipmentSlot.FEET, this.copyEquipment(source, EquipmentSlot.FEET, this.config.playerGhostArmorEnabled));
		entity.setStackInHand(Hand.MAIN_HAND, this.copyHandStack(source, Hand.MAIN_HAND, this.config.playerGhostHeldItemsEnabled));
		entity.setStackInHand(Hand.OFF_HAND, this.copyHandStack(source, Hand.OFF_HAND, this.config.playerGhostHeldItemsEnabled));
	}
	...
	private ItemStack copyEquipment(LivingEntity source, EquipmentSlot slot, boolean enabled) {
		...
		return source.getEquippedStack(slot).copy();
	}

	private ItemStack copyHandStack(LivingEntity source, Hand hand, boolean enabled) {
		if (!enabled || source == null) {
			return ItemStack.EMPTY;
		}

		return source.getStackInHand(hand).copy();
	}
	...
	private static Vec3d entityPosition(LivingEntity entity) {
	...
	private void drawHoverTooltip(DrawContext context, int mouseX, int mouseY) {
		...
		List<OrderedText> lines = this.textRenderer.wrapLines(Text.literal(this.displayedTooltip), wrapWidth - 12);
		int textWidth = 0;
		...
		x = MathHelper.clamp(x, 6, Math.max(6, this.width - boxWidth - 6));
		y = MathHelper.clamp(y, 6, Math.max(6, this.height - boxHeight - 6));
		...
		context.drawStrokedRectangle(x, y, boxWidth, boxHeight, tooltipColor(COLOR_PURPLE, easedOpacity));
		int textY = y + 6;
		for (OrderedText line : lines) {
			context.drawText(this.textRenderer, line, x + 6, textY, tooltipColor(COLOR_TEXT_MUTED, easedOpacity), true);
			textY += 10;
		}
	}
	...
	private void drawSwatches(DrawContext context) {
		for (Swatch swatch : this.swatches) {
			...
			context.drawStrokedRectangle(swatch.x() - 1, swatch.y() - 1, swatch.size() + 2, swatch.size() + 2,
	...
	private void drawOptionScrollFades(DrawContext context) {
		drawScrollFades(
			context,
			this.optionsX - 2,
	...
	private static void drawScrollFades(DrawContext context, int left, int top, int right, int bottom, int scrollOffset, int maxScroll) {
		if (maxScroll <= 0 || bottom <= top || right <= left) {
			return;
		}

		int fadeHeight = MathHelper.clamp((bottom - top) / 5, 10, 24);
		if (scrollOffset > 0) {
	...
	private void computeLayout() {
		...
		if (this.currentTab == MainTab.EFFECTS) {
			...
			int previewAvailableHeight = footerTop - this.contentY - 4;
			this.previewWidth = 0;
			this.previewHeight = 0;
			this.previewX = 0;
			this.previewY = 0;
			if (this.contentWidth >= 308 && previewAvailableHeight >= 112) {
				int wantedPreviewWidth = MathHelper.clamp((int)(this.contentWidth * 0.36F), 132, 300);
				int minOptionsWidth = this.contentWidth < 420 ? 168 : this.contentWidth < 560 ? 220 : 270;
			...
			this.optionsWidth = this.previewWidth > 0 ? Math.max(168, this.previewX - this.optionsX - previewGap) : this.contentWidth;
			this.optionsScrollTop = this.optionsY + 26;
		} else {
			this.previewX = 0;
			this.previewY = 0;
			this.previewWidth = 0;
	...
	private void togglePreviewPaused() {
		if (this.previewPaused) {
			...
			this.previewStartedAt = Util.getMeasuringTimeMs() - (long)(this.pausedPreviewProgress * durationMillis);
			this.previewPaused = false;
		} else {
	...
	private static Text movementText(KohsDeathEffectsConfig.GhostMovementMode mode) {
		return Text.literal(mode == KohsDeathEffectsConfig.GhostMovementMode.STATIC ? "Static" : "Rising");
	}
	...
	private static void drawCenteredButtonText(DrawContext context, PurpleButtonWidget button, int color) {
		MinecraftClient client = MinecraftClient.getInstance();
		String label = client.textRenderer.trimToWidth(button.getMessage().getString(), Math.max(8, button.getWidth() - 10));
	...
	private static final class PurpleIntSliderWidget extends SliderWidget {
		...
		private PurpleIntSliderWidget(
			...
			super(x, y, width, height, Text.literal(formatter.apply(snap(initialValue, minimum, maximum, step))), normalize(initialValue, minimum, maximum, step));
		...
		protected void updateMessage() {
			this.setMessage(Text.literal(this.formatter.apply(this.currentValue)));
		}
		...
		public void renderWidget(DrawContext context, int mouseX, int mouseY, float deltaTicks) {
			...
			context.drawStrokedRectangle(this.getX(), this.getY(), this.getWidth(), this.getHeight(), border);
			...
			MinecraftClient client = MinecraftClient.getInstance();
			String label = client.textRenderer.trimToWidth(this.getMessage().getString(), Math.max(8, this.getWidth() - 16));
			context.drawCenteredTextWithShadow(client.textRenderer, Text.literal(label), this.getX() + this.getWidth() / 2, this.getY() + (this.getHeight() - 8) / 2, COLOR_TEXT);
		}

		private static int snap(int value, int minimum, int maximum, int step) {
			int safeStep = Math.max(1, step);
			int clamped = MathHelper.clamp(value, minimum, maximum);
			int snapped = minimum + Math.round((clamped - minimum) / (float)safeStep) * safeStep;
			return MathHelper.clamp(snapped, minimum, maximum);
		}
	...
	private static final class PurpleButtonWidget extends ClickableWidget {
		private final ButtonTone tone;
		private final PurplePressAction onPress;

		private PurpleButtonWidget(int x, int y, int width, int height, Text message, PurplePressAction onPress, ButtonTone tone) {
	...
	private static final class MorphToScreen extends Screen {
		...
		protected void init() {
			...
			this.searchField = new TextFieldWidget(this.textRenderer, this.panelX + 14, this.panelY + 51, this.panelWidth - 28, 20, Text.literal("Search mob"));
			this.searchField.setMaxLength(48);
			this.searchField.setDrawsBackground(false);
			this.searchField.setEditableColor(COLOR_TEXT);
			this.searchField.setPlaceholder(Text.literal("Search mob"));
		...
		public void render(DrawContext context, int mouseX, int mouseY, float deltaTicks) {
			...
			context.drawText(this.textRenderer, Text.literal("Current mob: " + this.textRenderer.trimToWidth(MorphMobCatalog.selectedName(this.parent.config.morphEntityTypeId).getString(), Math.max(40, this.panelWidth - 124))), this.panelX + 14, this.panelY + 32, COLOR_TEXT_MUTED, false);
			...
			context.drawStrokedRectangle(this.searchField.getX() - 2, this.searchField.getY() - 2, this.searchField.getWidth() + 4, this.searchField.getHeight() + 4, COLOR_BORDER);
		...
		private void drawMobPreviews(DrawContext context) {
			...
			for (MorphMobCatalog.MobOption option : this.filteredOptions()) {
				if (y + 30 >= this.listTop && y <= this.listBottom) {
					...
					context.drawStrokedRectangle(previewLeft, y, 30, 30, option.id().equals(this.parent.config.morphEntityTypeId) ? COLOR_PURPLE : COLOR_BORDER);
		...
		private void drawMobRows(DrawContext context, int mouseX, int mouseY) {
			...
			for (MorphMobCatalog.MobOption option : this.filteredOptions()) {
				if (y + 30 >= this.listTop && y <= this.listBottom) {
					...
					String prefix = selected ? "> " : "";
					String label = this.textRenderer.trimToWidth(prefix + option.name().getString(), Math.max(24, rowWidth - 12));
					context.fill(rowX, y, rowX + rowWidth, y + 30, fill);
					context.drawStrokedRectangle(rowX, y, rowWidth, 30, border);
					context.drawText(this.textRenderer, Text.literal(label), rowX + 7, y + 11, COLOR_TEXT, false);
				}
				y += ROW_HEIGHT;
			}
		}
		...
		private void drawListScrollFades(DrawContext context) {
			drawScrollFades(
				context,
				this.panelX + 10,
				this.listTop,
	...
	private static final class MorphSoundWarningScreen extends Screen {
		...
		protected void init() {
			...
			this.addDrawableChild(new PurpleButtonWidget(this.panelX + 14, buttonY, buttonWidth, 20, Text.literal("Back"), button -> this.close(), ButtonTone.NORMAL));
			this.addDrawableChild(new PurpleButtonWidget(this.panelX + this.panelWidth - 14 - buttonWidth, buttonY, buttonWidth, 20, Text.literal("Continue"), button -> {
		...
		public void render(DrawContext context, int mouseX, int mouseY, float deltaTicks) {
			...
			context.drawCenteredTextWithShadow(this.textRenderer, Text.literal(this.textRenderer.trimToWidth("Enabling Mob sound will disable", textWidth)), this.panelX + this.panelWidth / 2, this.panelY + 44, COLOR_TEXT_MUTED);
			context.drawCenteredTextWithShadow(this.textRenderer, Text.literal(this.textRenderer.trimToWidth("the selected custom death sound.", textWidth)), this.panelX + this.panelWidth / 2, this.panelY + 58, COLOR_TEXT_MUTED);
			super.render(context, mouseX, mouseY, deltaTicks);
		}
	...
	private static final class CustomSoundWarningScreen extends Screen {
		...
		public void render(DrawContext context, int mouseX, int mouseY, float deltaTicks) {
			...
			context.drawCenteredTextWithShadow(this.textRenderer, this.title, this.panelX + this.panelWidth / 2, this.panelY + 14, COLOR_TEXT);
			int textWidth = this.panelWidth - 28;
			context.drawCenteredTextWithShadow(this.textRenderer, Text.literal(this.textRenderer.trimToWidth("Enabling Custom sound will disable", textWidth)), this.panelX + this.panelWidth / 2, this.panelY + 44, COLOR_TEXT_MUTED);
			context.drawCenteredTextWithShadow(this.textRenderer, Text.literal(this.textRenderer.trimToWidth("Morph mob sound.", textWidth)), this.panelX + this.panelWidth / 2, this.panelY + 58, COLOR_TEXT_MUTED);
			super.render(context, mouseX, mouseY, deltaTicks);
		}
