...
public final class KohsDeathEffectsConfigScreen extends Screen {
	...
	private void drawSilhouetteOptions(DrawContext context) {
		...
		context.drawBorder(fieldX - 2, fieldY - 2, fieldWidth + 4, 24, COLOR_BORDER);
	...
	private void drawKidsPreviewImage(DrawContext context, int left, int top, int right, int bottom) {
		...
		context.drawGuiTexture(RenderLayer::getGuiTextured, KIDS_PREVIEW_TEXTURE_ID, imageX, imageY, imageWidth, imageHeight);
	}
	...
	private void drawKidsPreviewLegacy(DrawContext context, int left, int top, int right, int bottom, float progress) {
		...
		if (showTrain) {
			...
			this.drawPlayerSkin(
				context,
				draggedModel,
				this.currentPreviewSkin().texture(),
				draggedScale,
				6.0F,
				facingYaw,
				0.96F,
	...
	private void drawSilhouetteModel(DrawContext context, int left, int top, int right, int bottom, float scale, int alpha) {
		...
		this.drawPlayerSkin(
			...
			right,
			bottom,
			ColorHelper.withAlpha(alpha, this.config.silhouetteColor)
		);
	}

	private void drawPlayerSkin(
		DrawContext context,
		...
		float availableHeight = Math.max(1.0F, bottom - top - 2.0F);
		float renderScale = Math.min(scale, availableHeight / 1.75F);
		var matrices = context.getMatrices();
		matrices.push();
		matrices.translate((left + right) * 0.5F, bottom - 1.0F, 200.0F);
		matrices.multiply(RotationAxis.POSITIVE_X.rotationDegrees(pitch));
		matrices.multiply(RotationAxis.POSITIVE_Y.rotationDegrees(yaw));
		matrices.scale(renderScale, -renderScale, renderScale);
		matrices.translate(0.0F, -verticalOffset, 0.0F);
		context.draw(consumers -> model.render(
			matrices,
			consumers.getBuffer(RenderLayer.getEntityTranslucent(texture, false)),
	...
	private PlayerEntityModel getSilhouettePreviewModel() {
		boolean slim = this.currentPreviewSkin().model() == SkinTextures.Model.SLIM;
	...
	private SkinTextures currentPreviewSkin() {
		if (this.client != null && this.client.player != null) {
			return this.client.player.getSkinTextures();
		}
	...
	private static void applyKidsDraggedPreviewOrientation(PlayerEntityModel model) {
		...
		rotatePreviewPart(model.hat, angle);
		rotatePreviewPart(model.jacket, angle);
		rotatePreviewPart(model.rightSleeve, angle);
		rotatePreviewPart(model.leftSleeve, angle);
		rotatePreviewPart(model.rightPants, angle);
		rotatePreviewPart(model.leftPants, angle);
	}

	private static void rotatePreviewPart(net.minecraft.client.model.ModelPart part, float angle) {
		float pivotY = 12.0F;
		float x = part.pivotX;
		float y = part.pivotY - pivotY;
		float cosine = MathHelper.cos(angle);
		float sine = MathHelper.sin(angle);
		part.pivotX = x * cosine - y * sine;
		part.pivotY = pivotY + x * sine + y * cosine;
		part.roll += angle;
	}
	...
	private static Identifier getSilhouettePreviewTexture(int argb) {
		...
		if (silhouettePreviewTexture == null) {
			silhouettePreviewTexture = new NativeImageBackedTexture(2, 2, false);
	...
	private void drawHoverTooltip(DrawContext context, int mouseX, int mouseY) {
		...
		context.drawBorder(x, y, boxWidth, boxHeight, tooltipColor(COLOR_PURPLE, easedOpacity));
		int textY = y + 6;
		for (OrderedText line : lines) {
	...
	private static final class MorphToScreen extends Screen {
		...
		@Override
		public boolean mouseClicked(double mouseX, double mouseY, int button) {
			if (button == 0 && this.handleMobRowClick(mouseX, mouseY)) {
				return true;
			}
		...
		private void drawMobRows(DrawContext context, int mouseX, int mouseY) {
			...
			for (MorphMobCatalog.MobOption option : this.filteredOptions()) {
				if (y + 30 >= this.listTop && y <= this.listBottom) {
					...
					context.drawBorder(rowX, y, rowWidth, 30, border);
