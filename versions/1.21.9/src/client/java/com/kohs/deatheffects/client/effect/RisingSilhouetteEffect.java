...
public final class RisingSilhouetteEffect {
	...
	public void render(MatrixStack matrices, VertexConsumerProvider consumers, OrderedRenderCommandQueue commandQueue, CameraRenderState cameraState) {
		...
		switch (this.mode) {
			case PLAYER_GHOST -> this.renderPlayerGhost(matrices, consumers, commandQueue, cameraState, alphaInt);
	...
	private void renderPlayerGhost(MatrixStack matrices, VertexConsumerProvider consumers, OrderedRenderCommandQueue commandQueue, CameraRenderState cameraState, int alpha) {
		...
		VertexConsumer skinVertices = consumers.getBuffer(RenderLayer.getEntityTranslucent(state.skinTextures.body().texturePath(), false));
	...
	private void renderVanillaFeaturePass(MatrixStack matrices, OrderedRenderCommandQueue commandQueue, CameraRenderState cameraState, PlayerEntityRenderState state, int alpha) {
		...
		RenderLayer suppressedBodyLayer = RenderLayer.getItemEntityTranslucentCull(state.skinTextures.body().texturePath());
	...
	private static class AlphaRenderCommandQueue implements RenderCommandQueue {
		...
		@Override
		public void submitDebugHitbox(MatrixStack matrices, EntityRenderState renderState, EntityHitboxAndView hitboxAndView) {
			this.delegate.submitDebugHitbox(matrices, renderState, hitboxAndView);
		}
