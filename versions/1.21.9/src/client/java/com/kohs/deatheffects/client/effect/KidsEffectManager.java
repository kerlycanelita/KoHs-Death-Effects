...
public final class KidsEffectManager {
	...
	public void render(MatrixStack matrices, VertexConsumerProvider consumers, CameraRenderState cameraState) {
		MinecraftClient client = MinecraftClient.getInstance();
		...
		for (Map.Entry<UUID, CarrierState> entry : this.carriers.entrySet()) {
			...
			this.renderCarrierKids(matrices, consumers, carrier, state, cameraPos, tickProgress);
		}
	}

	private void renderCarrierKids(MatrixStack matrices, VertexConsumerProvider consumers, AbstractClientPlayerEntity carrier, CarrierState state, Vec3d cameraPos, float tickProgress) {
		...
		for (int index = MAX_SHOULDER_KIDS; index < state.dolls.size(); index++) {
			...
			this.renderDoll(matrices, consumers, doll, position, trailYaw, bodyPitch, DRAGGED_DOLL_SCALE * ropeSizeMultiplier, frantic, false, cameraPos);
		...
		this.renderRope(matrices, consumers, ropeSegments, cameraPos, ropeSizeMultiplier);
	}
	...
	private void renderDoll(
		...
		int alphaInt = MathHelper.clamp((int)(alpha * 255.0F), 0, 255);
		VertexConsumer vertices = consumers.getBuffer(RenderLayer.getEntityTranslucent(doll.skin.body().texturePath(), false));
		matrices.push();
	...
	private void renderRope(MatrixStack matrices, VertexConsumerProvider consumers, List<RopeSegment> segments, Vec3d cameraPos, float sizeMultiplier) {
		if (segments.isEmpty()) {
			return;
		}
