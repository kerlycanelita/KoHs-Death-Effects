...
public final class KidsEffectManager {
	...
	void renderShoulderDolls(
		...
		float tickProgress = client.getRenderTickCounter().getTickDelta(false);
	...
	private void submitShoulderDoll(
		...
		PlayerEntityModel dollModel = this.getModel(skin.model() == SkinTextures.Model.SLIM);
		dollModel.setAngles(dollRenderState);
		showAllParts(dollModel);
		matrices.push();
	...
	private void renderDoll(
		...
		PlayerEntityModel model = this.getModel(doll.skin.model() == SkinTextures.Model.SLIM);
	...
	private PlayerEntityModel getModel(boolean slim) {
		if (slim) {
			if (this.slimModel == null) {
				this.slimModel = new KidsDollModel(MinecraftClient.getInstance().getEntityModelLoader().getModelPart(EntityModelLayers.PLAYER), true);
			}
			return this.slimModel;
		}

		if (this.regularModel == null) {
			this.regularModel = new KidsDollModel(MinecraftClient.getInstance().getEntityModelLoader().getModelPart(EntityModelLayers.PLAYER), false);
		}
