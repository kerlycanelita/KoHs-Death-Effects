...
public final class RisingSilhouetteEffect {
	...
	private static final float FAINT_START_HEIGHT = 2.35F;
	private static final float FAINT_GROUND_CONTACT_OFFSET = -0.18F;
	...
	private int faintFallDurationTicks = 24;
	...
	private void tickFaint() {
		...
		if (this.faintAnimationType == FaintAnimationType.CRAWL && !this.faintReachedTarget && client.player != null) {
			...
			if (distance > 0.0001) {
				float targetYaw = (float)Math.toDegrees(Math.atan2(delta.x, delta.z)) + 180.0F;
			...
			if (this.ageTicks < impactTick + FAINT_CRAWL_PREP_TICKS) {
				return;
			}
	...
	private void resolveFaintSurface() {
		...
		if (client.world == null) {
			this.faintSurfaceOffsetY = 0.0F;
			return;
		}

		double surfaceY = this.position.y;
		boolean found = false;
		int startY = MathHelper.floor(this.position.y + 1.0);
		int minY = Math.max(client.world.getBottomY(), startY - 96);
		for (int y = startY; y >= minY; y--) {
			BlockPos blockPos = BlockPos.ofFloored(this.position.x, y, this.position.z);
		...
		this.faintSurfaceOffsetY = found ? (float)(surfaceY - this.position.y) : 0.0F;
		float fallDistance = Math.max(0.75F, FAINT_START_HEIGHT - this.faintSurfaceOffsetY);
		this.faintFallDurationTicks = MathHelper.clamp((int)(12.0F + Math.sqrt(fallDistance) * 10.0F), 20, 78);
	}
	...
	public void render(WorldRenderContext context) {
		...
		float tickProgress = MinecraftClient.getInstance().getRenderTickCounter().getTickDelta(false);
		...
		Vec3d cameraPos = context.camera() != null ? context.camera().getPos() : Vec3d.ZERO;
	...
	private float getFaintYOffset(float elapsedTicks) {
		...
		return MathHelper.lerp(easeInCubic(fall), FAINT_START_HEIGHT, this.faintSurfaceOffsetY)
			+ MathHelper.lerp(smoothStep(settle), 0.0F, FAINT_GROUND_CONTACT_OFFSET);
	}
	...
	private void renderPlayerGhost(MatrixStack matrices, VertexConsumerProvider consumers, int alpha) {
		...
		this.renderVanillaFeaturePass(matrices, consumers, state, alpha);
	}
	...
	private void renderMorph(MatrixStack matrices, VertexConsumerProvider consumers, int alpha, float tickDelta) {
		...
		VertexConsumerProvider alphaConsumers = alpha >= 255 ? consumers : new AlphaVertexConsumerProvider(consumers, alpha / 255.0F, null);
	...
	private void renderFaintModel(MatrixStack matrices, VertexConsumer vertices, int argb, float progress, float elapsedTicks) {
		...
		float bodyPitch = this.faintAnimationType == FaintAnimationType.CRAWL
			? MathHelper.lerp(crawlAmount, 74.0F, -88.0F)
			: 74.0F;
	...
	private float getFaintCrawlAmount(float elapsedTicks) {
		...
		float impactTicks = this.faintFallDurationTicks + FAINT_SETTLE_TICKS;
		return smoothStep(MathHelper.clamp((elapsedTicks - impactTicks) / (float)FAINT_CRAWL_PREP_TICKS, 0.0F, 1.0F));
	}
	...
	private void renderVanillaFeaturePass(MatrixStack matrices, VertexConsumerProvider consumers, PlayerEntityRenderState state, int alpha) {
		PlayerEntityRenderer playerRenderer = this.pose.renderer();
		...
		playerRenderer.getModel().setAngles(state);
		VertexConsumerProvider featureConsumers = alpha >= 255
			? consumers
			: new AlphaVertexConsumerProvider(consumers, alpha / 255.0F, null, true);
		...
		for (FeatureRenderer<?, ?> feature : accessor.kohsDeathEffects$getFeatures()) {
			this.renderFeature(feature, matrices, featureConsumers, state);
		}
		matrices.pop();
	}

	@SuppressWarnings({"rawtypes", "unchecked"})
	private void renderFeature(FeatureRenderer<?, ?> feature, MatrixStack matrices, VertexConsumerProvider consumers, PlayerEntityRenderState state) {
	...
	private record PoseSnapshot(PlayerEntityRenderState state, boolean slim, Vec3d velocity, PlayerEntityRenderer renderer) {
		private static PoseSnapshot from(PlayerEntity player, KohsDeathEffectsConfig config) {
			...
			if (player instanceof AbstractClientPlayerEntity clientPlayer) {
				...
				if (renderer instanceof PlayerEntityRenderer playerRenderer) {
					...
					return new PoseSnapshot(state, slim, player.getVelocity(), playerRenderer);
				}
			}

			captureFallback(player, state);
			prepareForEffect(state, config);
			return new PoseSnapshot(state, slim, player.getVelocity(), null);
		}
		...
		private static void captureFallback(PlayerEntity player, PlayerEntityRenderState state) {
			...
			state.yawDegrees = MathHelper.wrapDegrees(player.getHeadYaw() - player.getBodyYaw());
		...
		private static void prepareForEffect(PlayerEntityRenderState state, KohsDeathEffectsConfig config) {
			...
			if (config.deathEffectMode == DeathEffectMode.PLAYER_GHOST) {
				...
				if (!config.playerGhostHeldItemsEnabled) {
					state.rightHandStack = ItemStack.EMPTY;
					state.leftHandStack = ItemStack.EMPTY;
					state.rightHandItemModel = null;
	...
	private static final class AlphaVertexConsumerProvider implements VertexConsumerProvider {
		...
		private RenderLayer remapLayer(RenderLayer layer) {
			...
			try {
				Identifier texture = Identifier.of(description.substring(textureStart, textureEnd));
