...
public final class RisingSilhouetteEffect {
	...
	public void render(LevelRenderContext context) {
		...
		switch (this.mode) {
			case PLAYER_GHOST -> this.renderPlayerGhost(matrices, commandQueue, cameraState, alphaInt);
			case RAGDOLL -> this.renderRagdoll(matrices, commandQueue, alphaInt, progress, elapsedTicks);
			case KIDS -> {
			}
			case MORPH -> this.renderMorph(matrices, commandQueue, cameraState, alphaInt, tickProgress);
			case SILHOUETTE -> this.renderSilhouette(matrices, commandQueue, alphaInt);
		}
	...
	private void renderSilhouette(PoseStack matrices, SubmitNodeCollector commandQueue, int alpha) {
		int red = this.color >> 16 & 0xFF;
		int green = this.color >> 8 & 0xFF;
		int blue = this.color & 0xFF;
		matrices.pushPose();
		matrices.scale(this.scale, this.scale, this.scale);
		this.renderBasePlayerModel(
			matrices,
			commandQueue,
			RenderTypes.entityTranslucentEmissive(getWhiteTexture(), false),
			ARGB.color(alpha, red, green, blue)
		);
		matrices.popPose();
	}

	private void renderPlayerGhost(PoseStack matrices, SubmitNodeCollector commandQueue, CameraRenderState cameraState, int alpha) {
	...
	private void renderRagdoll(PoseStack matrices, SubmitNodeCollector commandQueue, int alpha, float progress, float elapsedTicks) {
		AvatarRenderState state = this.pose.state();
		this.renderFaintModel(
			matrices,
			commandQueue,
			RenderTypes.entityTranslucent(state.skin.body().texturePath(), false),
			ARGB.color(alpha, 255, 255, 255),
	...
	private void renderFaintModel(PoseStack matrices, SubmitNodeCollector commandQueue, RenderType renderLayer, int argb, float progress, float elapsedTicks) {
		AvatarRenderState state = this.pose.state();
		...
		commandQueue.submitModelPart(this.model.root(), matrices, renderLayer, FULL_BRIGHT, OverlayTexture.NO_OVERLAY, null, argb, null, 0);
		matrices.popPose();
	}
	...
	private void renderBasePlayerModel(PoseStack matrices, SubmitNodeCollector commandQueue, RenderType renderLayer, int argb) {
		AvatarRenderState state = this.pose.state();
		this.model.setupAnim(state);
	...
	private static class AlphaRenderCommandQueue implements OrderedSubmitNodeCollector {
		...
		@Override
		public void submitNameTag(PoseStack matrices, Vec3 nameLabelPos, int y, Component label, boolean notSneaking, int light, CameraRenderState cameraState) {
			this.delegate.submitNameTag(matrices, nameLabelPos, y, label, notSneaking, light, cameraState);
		}
		...
		public void submitModelPart(
			...
			this.delegate.submitModelPart(part, matrices, renderLayer, light, overlay, sprite, this.fadeColor(tintedColor), crumblingOverlay, outlineColor);
		}

		@Override
		public void submitMovingBlock(PoseStack matrices, MovingBlockRenderState state, int outlineColor) {
			this.delegate.submitMovingBlock(matrices, state, outlineColor);
		}
		...
		@Override
		public void submitBreakingBlockModel(PoseStack matrices, List<BlockStateModelPart> parts, int overlay) {
			this.delegate.submitBreakingBlockModel(matrices, parts, overlay);
		}

		@Override
		public void submitShapeOutline(PoseStack matrices, VoxelShape shape, RenderType renderLayer, int color, float lineWidth, boolean depthTest) {
			this.delegate.submitShapeOutline(matrices, shape, renderLayer, this.fadeColor(color), lineWidth, depthTest);
		}
		...
		public void submitCustomGeometry(PoseStack matrices, RenderType renderLayer, SubmitNodeCollector.CustomGeometryRenderer customRenderer) {
			this.delegate.submitCustomGeometry(matrices, renderLayer, (pose, vertices) ->
				customRenderer.render(pose, new AlphaVertexConsumer(vertices, this.alphaMultiplier))
			);
		}
		...
		@Override
		public void submitGizmoPrimitives(DrawableGizmoPrimitives.Group group, CameraRenderState cameraState, boolean depthTest) {
			this.delegate.submitGizmoPrimitives(group, cameraState, depthTest);
		}
