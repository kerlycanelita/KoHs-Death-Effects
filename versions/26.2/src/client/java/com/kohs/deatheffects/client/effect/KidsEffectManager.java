...
public final class KidsEffectManager {
	...
	private PlayerModel regularModel;
	...
	private void receive(KidsEventPayload payload, Minecraft client) {
	...
	private void addDoll(Minecraft client, UUID carrierUuid, UUID killUuid, UUID victimUuid, String victimName, LocalSettings settings) {
		...
		if (state.dolls.size() >= limit) {
			if (settings.mode() == KidsMode.CUMULATIVE && client.player != null && carrierUuid.equals(client.player.getUUID())) {
				client.gui.hud.setOverlayMessage(Component.literal("Kids limit reached: 20 players"), false);
			}
			return;
		}
		...
		AbstractClientPlayer onlineVictim = findPlayer(client, victimUuid);
		if (onlineVictim != null) {
			doll.skin = onlineVictim.getSkin();
		} else {
			client.getSkinManager().get(profile).thenAccept(result -> client.execute(() -> {
				if (result.isPresent()) {
	...
	private void tick(Minecraft client) {
		...
		for (Iterator<Map.Entry<UUID, CarrierState>> iterator = this.carriers.entrySet().iterator(); iterator.hasNext();) {
			...
			state.removeExpiredDolls();
			AbstractClientPlayer carrier = findPlayer(client, entry.getKey());
			if (carrier == null) {
				state.missingCarrierTicks++;
			} else {
				state.missingCarrierTicks = 0;
				state.recordCarrierPosition(new Vec3(carrier.getX(), carrier.getY(), carrier.getZ()), carrier.yBodyRot);
			}
	...
	void renderShoulderDolls(
		...
		int light,
		AvatarRenderState carrierRenderState,
		PlayerModel carrierModel
	) {
		...
		Entity entity = client.level.getEntity(carrierRenderState.id);
		if (!(entity instanceof AbstractClientPlayer carrier)) {
			return;
		}
		if (client.player != null
	...
	private void submitShoulderDoll(
		...
		dollRenderState.skin = skin;
		dollRenderState.shoulderPose = true;
		dollRenderState.shoulderWave = wave;

		PlayerModel dollModel = this.getModel(skin.model() == PlayerModelType.SLIM);
		matrices.pushPose();
		carrierModel.body.translateAndRotate(matrices);
		...
		commandQueue.submitModel(
			...
			dollRenderState,
			matrices,
			RenderTypes.entityTranslucent(skin.body().texturePath(), false),
			light,
	...
	private void render(LevelRenderContext context) {
		...
		if (client.level == null || matrices == null || commandQueue == null || this.carriers.isEmpty()) {
			return;
		}
		...
		for (Map.Entry<UUID, CarrierState> entry : this.carriers.entrySet()) {
			AbstractClientPlayer carrier = findPlayer(client, entry.getKey());
			CarrierState state = entry.getValue();
			if (carrier == null || carrier.isDeadOrDying() || state.dolls.isEmpty()) {
				continue;
			}
	...
	private void renderCarrierKids(LevelRenderContext context, AbstractClientPlayer carrier, CarrierState state, Vec3 cameraPos, float tickProgress) {
		Vec3 carrierPos = carrier.getPosition(tickProgress);
		float bodyYaw = Mth.rotLerp(tickProgress, carrier.yBodyRotO, carrier.yBodyRot);
		double crouchOffset = carrier.isShiftKeyDown() ? -0.18 : 0.0;
		...
		Vec3 behind = behindForYaw(bodyYaw);
		Vec3 previousRopePoint = carrierPos.add(behind.scale(0.16)).add(0.0, 0.86 + crouchOffset, 0.0);
		...
		for (int index = MAX_SHOULDER_KIDS; index < state.dolls.size(); index++) {
			...
			if (Double.isNaN(doll.smoothedGroundY)) {
				...
				doll.smoothedGroundY = Mth.lerp(0.30, doll.smoothedGroundY, groundY);
			}
	...
	private void renderDoll(
		...
		PlayerModel model = this.getModel(doll.skin.model() == PlayerModelType.SLIM);
		AvatarRenderState state = createDollRenderState();
		...
		if (bodyPitch != 0.0F) {
			matrices.mulPose(Axis.XP.rotationDegrees(bodyPitch));
		}
		matrices.scale(scale, scale, scale);
		...
		context.submitNodeCollector().submitModelPart(
			model.root(),
			matrices,
			RenderTypes.entityTranslucent(doll.skin.body().texturePath(), false),
			FULL_BRIGHT,
			OverlayTexture.NO_OVERLAY,
			null,
	...
	private void renderRope(LevelRenderContext context, List<RopeSegment> segments, Vec3 cameraPos, float sizeMultiplier) {
		if (segments.isEmpty()) {
			return;
		}
		...
		context.submitNodeCollector().submitCustomGeometry(matrices, RenderTypes.linesTranslucent(), (entry, vertices) -> {
			for (RopeSegment segment : segments) {
				...
				int alpha = Mth.clamp((int)(segment.alpha() * 235.0F), 0, 235);
				vertices.addVertex(entry, (float)from.x, (float)from.y, (float)from.z)
					.setColor(ROPE_RED, ROPE_GREEN, ROPE_BLUE, alpha)
					.setNormal(entry, (float)direction.x, (float)direction.y, (float)direction.z)
					.setLineWidth(2.0F * sizeMultiplier);
				vertices.addVertex(entry, (float)to.x, (float)to.y, (float)to.z)
					.setColor(ROPE_RED, ROPE_GREEN, ROPE_BLUE, alpha)
					.setNormal(entry, (float)direction.x, (float)direction.y, (float)direction.z)
					.setLineWidth(2.0F * sizeMultiplier);
			}
	...
	private PlayerModel getModel(boolean slim) {
		if (slim) {
			if (this.slimModel == null) {
				this.slimModel = new KidsDollModel(Minecraft.getInstance().getEntityModels().bakeLayer(ModelLayers.PLAYER), true);
			}
			return this.slimModel;
		}

		if (this.regularModel == null) {
			this.regularModel = new KidsDollModel(Minecraft.getInstance().getEntityModels().bakeLayer(ModelLayers.PLAYER), false);
		}
	...
	private static void applyShoulderPose(PlayerModel model, float wave) {
		model.head.xRot = -0.08F;
		model.head.yRot = wave * 0.08F;
		model.body.xRot = 0.04F;
		model.rightArm.xRot = -1.55F + wave * 0.62F;
		model.rightArm.yRot = -0.18F;
		model.rightArm.zRot = 0.18F;
		model.leftArm.xRot = -1.55F - wave * 0.62F;
		model.leftArm.yRot = 0.18F;
		model.leftArm.zRot = -0.18F;
		model.rightLeg.xRot = -1.38F;
		model.rightLeg.yRot = 0.12F;
		model.leftLeg.xRot = -1.38F;
		model.leftLeg.yRot = -0.12F;
	}

	private static void applyDraggedPose(PlayerModel model, float frantic) {
	...
	private static AbstractClientPlayer findPlayer(Minecraft client, UUID uuid) {
		if (client.level == null) {
			return null;
		}
	...
	private static net.minecraft.client.multiplayer.ClientLevel clientWorld(AbstractClientPlayer player) {
		return (net.minecraft.client.multiplayer.ClientLevel)player.level();
	}

	private static Vec3 behindForYaw(float yawDegrees) {
		double radians = Math.toRadians(yawDegrees);
		return new Vec3(Math.sin(radians), 0.0, -Math.cos(radians));
	}

	private static float yawFromBehindDirection(Vec3 direction) {
	...
	private static Vec3 horizontalDirection(Vec3 from, Vec3 to) {
		...
		return new Vec3((to.x - from.x) / distance, 0.0, (to.z - from.z) / distance);
	}

	private static double findGroundY(net.minecraft.client.multiplayer.ClientLevel world, double x, double z, double startSurfaceY, double fallbackY) {
		int startY = Mth.floor(startSurfaceY);
		int minY = Math.max(world.getMinY(), startY - 16);
	...
	private static final class KidsDollRenderState extends AvatarRenderState {
		private boolean shoulderPose;
		private float shoulderWave;
	}

	private static final class KidsDollModel extends PlayerModel {
	...
	private record LocalSettings(boolean enabled, KidsMode mode, int timerSeconds, int ropeSizePercent, KidsTrainFacing trainFacing, boolean animationEnabled, boolean draggedHandMovementEnabled) {
		...
		private static LocalSettings fromPayload(KidsEventPayload payload) {
			...
			return new LocalSettings(
				true,
				mode,
				Mth.clamp(payload.timerSeconds(), minimumTimer, 301),
				Mth.clamp(payload.ropeSizePercent(), 100, 300),
				trainFacing,
	...
	private static final class CarrierState {
		...
		private Vec3 fallbackTrailDirection = new Vec3(0.0, 0.0, -1.0);
		...
		private void recordCarrierPosition(Vec3 position, float bodyYaw) {
			Vec3 current = new Vec3(position.x, position.y, position.z);
		...
		private void resetTrail(Vec3 position) {
		...
		private void trimTrail() {
			...
			Vec3 previous = null;
			for (Iterator<Vec3> iterator = this.trailPoints.iterator(); iterator.hasNext();) {
		...
		private TrailSample sampleTrail(Vec3 currentPosition, double requestedDistance, float bodyYaw) {
			Vec3 previous = currentPosition;
			Vec3 lastDirection = this.fallbackTrailDirection.lengthSqr() > 0.0001
				? this.fallbackTrailDirection
				: behindForYaw(bodyYaw);
			double accumulatedDistance = 0.0;
			boolean skippedNewestPoint = false;
			for (Vec3 point : this.trailPoints) {
				...
				Vec3 direction = horizontalDirection(previous, point);
			...
			return new TrailSample(previous.add(lastDirection.scale(remainingDistance)), lastDirection);
		}
	}

	private record TrailSample(Vec3 position, Vec3 direction) {
	}
	...
	private static final class DollState {
		...
		private DollState(UUID killUuid, UUID victimUuid, String victimName, PlayerSkin skin, int maxAgeTicks) {
		...
		private float alpha() {
			float spawnAlpha = smoothStep(Mth.clamp(this.ageTicks / 10.0F, 0.0F, 1.0F));
			if (this.maxAgeTicks < 0) {
				return spawnAlpha;
			}
			int remainingTicks = this.maxAgeTicks - this.ageTicks;
			float fadeAlpha = smoothStep(Mth.clamp(remainingTicks / (float)FADE_TICKS, 0.0F, 1.0F));
